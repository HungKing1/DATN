...
@tool
def delegate_task(description: str, law_uuid: Optional[str] = None) -> str:
    """Tạo task cho Paralegal Agent đi tìm kiếm thông tin theo mô tả.
    Args:
        description: Mô tả chi tiết vấn đề pháp lý cần tìm kiếm.
        law_uuid: UUID của bộ luật nếu biết, nếu không có thể để None.
    """
    ...
    async def search_law_database(
        ...
        try:
            ...
            for i, r in enumerate(results):
                ...
                chunks.append(f"--- Chunk {i+1} ---\nNguồn: {r.metadata.get('file_name', 'Unknown')}\nNội dung:\n{r.content}")
