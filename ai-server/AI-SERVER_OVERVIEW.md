...
─────────────────────────────────────────────────────
# Core
content          : TEXT     — embedding_text (bao gồm prefix + content để LLM có ngữ cảnh)
chunk_index      : INT
...
LegalArticleChunker.chunk(articles, doc_meta)
    ...
    ② size < 300 → merge với Điều tiếp (thuật toán nhóm theo LCP - Longest Common Path để tránh lặp prefix)
    ③ 300 ≤ size ≤ 600 → 1 chunk (Normal chunk)
    ④ size > 600 → split theo khoản dùng Balanced Partitioning để chia các chunk có size bằng nhau nhất
    ⑤ SAU KHI xong → build embedding_text = prefix + content
       prefix = "{loai_van_ban} {so_ky_hieu}: {ten_day_du}. {path hierarchy}"
    ⑥ Gán chunk.content = chunk.legal.embedding_text để đồng bộ lưu trữ vào Weaviate.
    │
    ▼
...
class LegalChunk(BaseModel):
    id: UUID
    content: str           # embedding_text (LLM đọc)
    chunk_index: int
