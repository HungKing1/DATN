...
- **Chat mode routing**: Tất cả tin nhắn đều đi qua Multi-Agent RAG pipeline (POST `/api/v1/query/agent`)
...
- Hoàn thiện UI Frontend để xóa mode selector (⚡/🧠) vì Backend đã xóa RAG Query Workflow.
